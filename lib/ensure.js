...
const getTranslator = (pkg) => {
  ...
  console.log(
    'TRANSLATOR PPPPPPPPPPPPPAAAAAAAATTHHH',
    _path.resolve(__dirname, `../translators/${pkg}`)
  )

  if (require('fs').existsSync(_path.resolve(__dirname, `../translators/${pkg}`))) {
...
const ensureInstalled = (pkg, installCommand, options = { progress: false, verbose: true }) => {
  ...
  if (!path || (path.split && ~path.split('\n')[0].indexOf('WindowsApps\\python'))) {
    ...
    // defaultOptions.env.PATH =
    //   defaultOptions.env.PATH + ';' + `${require('os').homedir()}\\scoop\\shims\\`
